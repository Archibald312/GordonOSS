...
export class EdgarClient {
    ...
    async getFilingIndex(
        ...
        // Best-effort: pick the primary doc. The submissions API
        // (getRecentFilings) returns an authoritative primary_document
        // field; callers should pass that through. This fallback only
        // fires when the caller has no other signal.
        //
        // We explicitly exclude SEC-generated render files (R\d+.htm —
        // the financial-fragment HTMLs derived from XBRL), FilingSummary,
        // MetaLinks, and the Financial_Report.xlsx. These come alphabetically
        // before the real primary in index.json and used to win the pick.
        const isAutoGenerated = (name: string) =>
            /^R\d+\.htm$/i.test(name) ||
            /^FilingSummary\.xml$/i.test(name) ||
            /^MetaLinks\.json$/i.test(name) ||
            /^Financial_Report\.xlsx$/i.test(name);
        const candidates = docs.filter((d) => !isAutoGenerated(d.name));
        const primary =
            candidates.find((d) =>
                /^(10-?k|10-?q|8-?k|s-?1)$/i.test(d.type),
            )?.name ??
            // Largest .htm that isn't an SEC R-file or summary — the actual
            // 10-K body is typically the largest HTML in the bundle.
            [...candidates.filter((d) => /\.htm$/i.test(d.name))].sort(
                (a, b) => b.size - a.size,
            )[0]?.name ??
            candidates[0]?.name ??
            docs[0]?.name ??
