...
import type { GordonDocument, GordonProject, GordonWorkflow } from "../shared/types";
import {
    getProject,
    listProjects,
...
interface Props {
    ...
    onAdd: (
        ...
        projectId?: string,
        documentIds?: string[],
        columnsConfig?: GordonWorkflow["columns_config"],
    ) => void;
    projects?: GordonProject[];
    /** When provided, skip the project/directory picker and show only these docs */
    projectDocs?: GordonDocument[];
    projectName?: string;
...
export function AddNewTRModal({
    ...
    const [projectDocs, setProjectDocs] = useState<GordonDocument[]>([]);
    const [loadingDocs, setLoadingDocs] = useState(false);

    // Full directory (when underProject is false)
    const [standaloneDocs, setStandaloneDocs] = useState<GordonDocument[]>([]);
    const [directoryProjects, setDirectoryProjects] = useState<GordonProject[]>(
        [],
    );
    ...
    // Workflow templates
    const [workflows, setWorkflows] = useState<GordonWorkflow[]>([]);
    ...
    const directoryFolders = isProjectMode
        ...
        : underProject
          ? []
          : directoryProjects;
    const flatProjectDocs: GordonDocument[] =
