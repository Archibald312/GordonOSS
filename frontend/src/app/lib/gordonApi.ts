/**
 * Gordon API client — all requests to the Node.js backend.
...
interface ServerMessage {
    ...
    annotations?: GordonCitationAnnotation[] | null;
    created_at: string;
}
...
export async function listProjects(): Promise<GordonProject[]> {
    return apiRequest<GordonProject[]>("/projects");
}
...
export async function getProject(projectId: string): Promise<GordonProject> {
    return apiRequest<GordonProject>(`/projects/${projectId}`);
}
...
): Promise<GordonFolder> {
    return apiRequest<GordonFolder>(`/projects/${projectId}/folders`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            name,
...
export async function renameProjectFolder(
    ...
    return apiRequest<GordonFolder>(
...
): Promise<GordonDocument> {
    return apiRequest<GordonDocument>(
...
export interface GordonDocumentVersion {
    id: string;
...
export async function listDocumentVersions(documentId: string): Promise<{
    ...
    versions: GordonDocumentVersion[];
}> {
...
export async function uploadDocumentVersion(
    ...
    return response.json() as Promise<GordonDocumentVersion>;
}
...
export async function renameDocumentVersion(
    ...
    return apiRequest<GordonDocumentVersion>(
...
export async function uploadProjectDocument(
    ...
    if (!response.ok) throw new Error(await response.text());
    return response.json() as Promise<GordonDocument>;
}
...
export async function listStandaloneDocuments(): Promise<GordonDocument[]> {
    return apiRequest<GordonDocument[]>("/single-documents");
}
...
export async function listChats(): Promise<GordonChat[]> {
    return apiRequest<GordonChat[]>("/chat");
}

export async function listProjectChats(projectId: string): Promise<GordonChat[]> {
    return apiRequest<GordonChat[]>(`/projects/${projectId}/chats`);
}

export async function getChat(chatId: string): Promise<GordonChatDetailOut> {
    const raw = await apiRequest<ServerChatDetailOut>(`/chat/${chatId}`);
    const messages: GordonMessage[] = raw.messages.map((m) => {
        if (m.role === "user") {
            return {
                role: "user",
...
type WorkflowType = GordonWorkflow["type"];

export async function listWorkflows(
    type: WorkflowType,
): Promise<GordonWorkflow[]> {
    return apiRequest<GordonWorkflow[]>(`/workflows?type=${type}`);
}

export async function getWorkflow(workflowId: string): Promise<GordonWorkflow> {
    return apiRequest<GordonWorkflow>(`/workflows/${workflowId}`);
}
...
export async function createWorkflow(payload: {
    ...
    return apiRequest<GordonWorkflow>("/workflows", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
    });
